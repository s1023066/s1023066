...
public static void Main()
{
	...
	Console.WriteLine("現在時刻:{0}年{1}月{2}日 {3} {4}:{5}:{6}", 
	dt.Year,dt.Month,dt.Day,dt.DayOfWeek,dt.Hour,dt.Minute,dt.Secound);
}
...
namespace Microsoft.Samples.Kinect.SkeletonBasics
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public static void Main()
        {
            ...
            StreamWriter sw = new StreamWriter(@"c:\test\hoge.txt", false, enc);
            sw.Write("私の戦闘力は");
            sw.WriteLite("{0}です", 530000);
            sw.Close();
        }
                ...
                if (skeletons.Length != 0)
                {
                    foreach (Skeleton skel in skeletons)
                    {
                        ...
                        if (skel.TrackingState == SkeletonTrackingState.Tracked)
                        {
                            ...
                            label1.Content = skel.Joints[JointType.Spine].Position.Z;
                            ...
                            if (dot >= 0)
                            {
                                dc.DrawRectangle(Brushes.White, null, new Rect(0.0, 0.0, RenderWidth, RenderHeight)); //背景
                                this.DrawBonesAndJoints(skel, dc); //骨格情報
                            }
                            if (dot >= 0.5)
                            {
                                dc.DrawRectangle(Brushes.Red, null, new Rect(0.0, 0.0, RenderWidth, RenderHeight)); //背景
                                this.DrawBonesAndJoints(skel, dc); //骨格情報
                            }
